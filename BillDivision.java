...
class Bill {
	//Declaring another method to perform operation
    public void division(int total,int count) {
    	//to find balance equal 
   count/=2;
   //compare the both value
   if(count==total){
       System.out.println("Bon Appetit");
   }// statement false execute this statement
   else {
...
public class BillDivision {
	...
	public static void main(String[] args) {
		...
		//Initializiting the size of array 
       System.out.print("Enter the size of array : ");
        int size=in.nextInt();
      //Initializiting the index that should add count 
        System.out.print("Enter the indext that should not calculate : ");
    int reject=in.nextInt();
    System.out.println("Enter the elements of array : ");
  //Initializiting the array and size
    int[] array=new int[size];
  //Initializiting count
    int count=0;
    //Initializiting the loop to travels till end of loop
    for(int index=0;index<size;index++){
        array[index]=in.nextInt();
        //index and reject value not to be equals
        if(index!=reject){
        	//make sum on count
            count+=array[index];
        }
    }
    System.out.print("Enter amount each shoul pay : ");
    //Initializiting both pays the money
    int pay=in.nextInt();
    //creating object for class
   Bill bill=new Bill();
   //passing argument for method
   bill.division(pay,count);
	}
